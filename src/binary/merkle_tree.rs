    ...
    StorageType: StorageInspect<TableType, Error = StorageError>,
{
    ...
    /// The root node is generated by joining all MMR peaks, where a peak is defined as the head of
    /// a balanced subtree. A tree can be composed of a single balanced subtree, in which case the
    /// tree is itself balanced, or several balanced subtrees, in which case the tree is imbalanced.
    /// Only nodes at the head of a balanced tree are persisted in storage; any node, including the
    /// root node, whose child is an imbalanced child subtree will not be saved in persistent
    /// storage. This is because node data for such nodes is liable to change as more leaves are
    /// pushed to the tree. Instead, intermediate nodes must be held in a temporary storage space.
    ///
    /// When calling `root_node`, callees must pass a mutable reference to a temporary storage space
    /// that will be used to hold any intermediate nodes that are created during root node
    /// calculation. At the end of the method call, this temporary storage space will be contain all
    /// intermediate nodes not held in persistent storage, and these nodes will be available to the
    /// callee.
    ///
